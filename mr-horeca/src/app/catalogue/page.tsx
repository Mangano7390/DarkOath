  ...
  return (
    <div className="container py-12 space-y-10">
      <div className="space-y-2">
        <div className="text-xs uppercase tracking-[0.2em] text-brand">B2B Pro</div>
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((c) => (
          <Link
            key={c.id}
            href={`/c/${c.slug}`}
            className="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm hover:bg-brand hover:text-white hover:border-brand transition"
          >
            {c.name}
